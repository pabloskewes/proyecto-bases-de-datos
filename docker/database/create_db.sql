...
ALTER TABLE pudin.PasaPor
    ...
    t_comuna, t_sentido) REFERENCES pudin.Trazado (calle, comuna, sentido);
...
GRANT SELECT ON ALL TABLES IN SCHEMA pudin TO pudinpoof;
GRANT CONNECT ON DATABASE cc3201 TO pudinpoof;


COPY pudin.servicio FROM '/data/servicio.csv' DELIMITER ',' CSV HEADER;
COPY pudin.vehiculo FROM '/data/vehiculo.csv' DELIMITER ',' CSV HEADERCOPY pudin.pasapor (s_folio, s_region, r_nombre_recorrido, t_calle, t_comuna, t_sentido, orden) FROM '/data/pasapor.csv' DELIMITER ',' CSV HEADER;;
COPY pudin.recorrido (s_region, s_folio, nombre_recorrido, id_origen, id_destino) FROM '/data/recorrido.csv' DELIMITER ',' CSV HEADER;
COPY pudin.lugar (id, nombre, comuna, domicilio, region, latitud, longitud) FROM '/data/lugar.csv' DELIMITER ',' CSV HEADER;
COPY pudin.trazado (sentido, calle, comuna) FROM '/data/trazado.csv' DELIMITER ',' CSV HEADER;
COPY pudin.pasapor (s_folio, s_region, r_nombre_recorrido, t_calle, t_comuna, t_sentido, orden) FROM '/data/pasapor.csv' DELIMITER ',' CSV HEADER;